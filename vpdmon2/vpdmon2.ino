...
time_t delayCC = now() - DELAY_CC;

volatile uint8_t loopMode = 0;

SoftwareSerial Serial1((uint8_t)3, (uint8_t)4); // RX, TX
...
// initialize ESP module
  ...
  // read EEPROM
  uint8_t *buf;
  buf = (uint8_t *)&eEPROMObject.last_known_time;
  for (uint8_t i = 0; i < sizeof(eEPROMObject); i++) {
    *(buf+i) = EEPROM.read(i);
  } 
  serialClockDisplay(eEPROMObject.last_known_time);
  setTime(eEPROMObject.last_known_time+300);
  serialClockDisplay();
...
void displayMessage(String msg) {
  ...
  displayMessage(msg, lastMsg, 0, 120);
  lastMsg = msg;
}

void displayMessage(String msg, String lastMsg, uint8_t x, uint8_t y) {
  displayMessage(msg, lastMsg, x, y, WHITE);
}

void displayMessage(String msg, String lastMsg, uint8_t x, uint8_t y, uint16_t c) {
  displayMessage(msg, lastMsg, x, y, c, 1);
}

void displayMessage(String msg, String lastMsg, uint8_t x, uint8_t y, uint16_t c, uint8_t s) {
  Serial.println(msg);
  display.setTextScale(s);
...
void loop() {
  ...
  switch(loopMode) {
    ...
    case 1:
    case 2:
      displayMessage(String("^^^^").substring(0, 4 * (loopMode - 1)), "", 24, 56, BLACK, 1);
...
//      display.print(String("^^^^").substring(0, 4 * (loopMode - 1)));
    ...
    default:
      displayMessage(String("^^^^^^^^"), "", 24, 56, BLACK, 1);
//      display.setCursor(24, 56);
//      display.setTextScale(1);
//      display.setTextColor(BLACK);
...
//  char timeServer[] = "time.nist.gov";  // NTP server
  ...
  // wait for a reply for 3000 miliseconds
  unsigned long startMs = millis();
  while (!Udp.available() && (millis() - startMs) < 3000) {}
  ...
  if (Udp.parsePacket()) {
    ...
    time_t t = highWord << 16 | lowWord;
    displayMessage(String("ntp_t: ") + t);
    delay(300);
    ...
    t = t - seventyYears;
    // print Unix time:
    displayMessage(String("unix_t: ") + t);
...
void sendNTPpacket(IPAddress ntpSrv)
{
...
void wifiConnect() {
  ...
  while ( status != WL_CONNECTED) {
    displayMessage(String("Connecting"));
    // Connect to WPA/WPA2 network
    status = WiFi.begin(ssid, pass);
  }

  displayMessage(ssid + String(":") + WiFi.RSSI());
...
void displayTime() {
  ...
  digitalClockDisplay(lastTime, BLACK);
  lastTime = now() % 86400;
...
//    EEPROM.get(0, eEPROMObject);
          Serial.println(eEPROMObject.last_known_time);
        }
...
void writeEEPROM(time_t *pctime) {
  eEPROMObject.last_known_time = *pctime = now();
  uint8_t *buf;
  buf = (uint8_t *)&eEPROMObject.last_known_time;
  for (uint8_t i = 0; i < sizeof(eEPROMObject); i++) {
  ...
  delay(2000);
  s_EEPROMObject e;
  buf = (uint8_t *)&e.last_known_time;
  for (uint8_t i = 0; i < sizeof(e); i++) {
    *(buf+i) = EEPROM.read(i);
  } 
  serialClockDisplay(e.last_known_time);
}
